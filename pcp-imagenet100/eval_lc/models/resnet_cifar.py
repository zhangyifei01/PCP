...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.bn1(out)
        # print(out.shape) # torch.Size([128, 64, 32, 32])
        out = F.relu(out)
        out = self.layer1(out)
        # print(out.shape) # torch.Size([128, 64, 32, 32])
        out = self.layer2(out)
        # print(out.shape) # torch.Size([128, 128, 16, 16])
        out = self.layer3(out)
        # print(out.shape) # torch.Size([128, 256, 8, 8])
        out = self.layer4(out)
        # print(out.shape) # torch.Size([128, 512, 4, 4])
        # print('*'*60)
        # print(out.shape)
        out = F.avg_pool2d(out, 4)
        # print(out.shape) # torch.Size([128, 512, 1, 1])
        # print('#'*60)
...
def test():
    net = ResNet18()
    fd  = int(net.linear.weight.size()[1])
    fd1  = int(net.linear.weight.size()[0])
    print(net)
    print(fd)
    net.linear = None
